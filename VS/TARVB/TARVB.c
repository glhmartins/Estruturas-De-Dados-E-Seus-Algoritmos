...
void aux_ni(TARVB *a, int *count){
  if(!a) return;
  int i;
  for(i = 0; i<=a->nchaves; i++){
    if(!a->folha) aux_ni(a->filho[i], count);
    if((!a->folha) && (i == a->nchaves-1)) (*count)++;
  }
}
...
void aux_nf(TARVB *a, int *count){
  if(!a) return;
  int i;
  for(i = 0; i<=a->nchaves; i++){
    if(!a->folha) aux_nf(a->filho[i], count);
    if((a->folha) && (i == a->nchaves-1)) (*count)++;
  }
}
...
TARVB *maior(TARVB *a){
  if(!a) return a;
  TARVB *aux = a;
  while(!aux->folha) aux = aux->filho[aux->nchaves];
  return aux;
}
...
TARVB *retira_pares(TARVB *a){
  ...
  for(i = 0; i<=a->nchaves; i++){
    ...
    if(i < a->nchaves){
      if(a->chave[i]%2 == 0) a = TARVB_Retira(a, a->chave[i], (sizeof(a->filho)/sizeof(TARVB))*2);
    }
  }
  return a;
}
...
int max_elem(TARVB *a, int elem){
  TARVB *aux = a;
  while(!aux->folha) aux = aux->filho[aux->nchaves];
  if(elem == aux->chave[aux->nchaves-1]) return 1;
  return 0;
}

int suc(TARVB *a, int elem){
  if(!a) return -1; // Não tem arvore
  if(max_elem(a, elem)) return __INT_MAX__;
  TARVB *aux = TARVB_Busca(a, elem);
  int i;
  for(i = 0; i<aux->nchaves; i++){
    if(aux->chave[i] == elem && i!=aux->nchaves-1) return aux->chave[i+1];
  }
}

TLSE *mN(TARVB *a, int n){
  if(!a) return NULL;
  TLSE *l = TLSE_inicializa(), *auxl = TLSE_inicializa();
  int i;
  TARVB *aux = a;
  for(i = 0; i<=aux->nchaves; i++){
    auxl = mN(aux->filho[i], n);
    while(auxl){
      if(auxl->info>n) l = TLSE_insere(l, auxl->info);
      auxl = auxl->prox;
    }
    if(i<aux->nchaves){
      if(aux->chave[i]>n) l = TLSE_insere(l, aux->chave[i]);
    }
  }
  return l;
}
...
TLSEINT *misc(TARVB *a, int elem){
  ...
  TARVB *aux = a;
  int i = 0;
  while((aux->chave[i]!=elem) && (aux)){
    while(aux->chave[i]<elem) i++;
    if(aux->chave[i] == elem) {
      l = TLSEINT_insere(l, aux->chave[i], aux->chave[i]);
      i = 0;
      break;
    }
    else l = TLSEINT_insere(l, aux->chave[0], aux->chave[aux->nchaves-1]);
    aux = aux->filho[i];
    i = 0;
  }
  if(aux->chave[i] == elem) l = TLSEINT_insere(l, aux->chave[i], aux->chave[i]);
  TLSEINT_imprime(l);
  return l;
}
