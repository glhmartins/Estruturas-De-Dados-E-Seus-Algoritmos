...
void aux_ni(TARVBM *a, int *count){
  if(!a) return;
  int i;
...
void aux_nf(TARVBM *a, int *count){
  if(!a) return;
  int i;
  for(i = 0; i<=a->nchaves; i++){
...
TARVBM *retira_pares(TARVBM *a){
  ...
  for(i = 0; i<=a->nchaves; i++){
    ...
    if((a->folha)){
      ...
      if((a->chave[i]%2==0)) a = TARVBM_retira(a, a->chave[i], (sizeof(a->filho)/sizeof(TARVBM))*2);
      } 
    }
  }
  return a;
...
int max_suc(TARVBM *a, int elem){
  if(!a) return 0;
  TARVBM *aux = a;
  while(!aux->folha) aux = aux->filho[aux->nchaves];
  if(aux->chave[aux->nchaves-1] == elem) return 1;
  return 0;
}

int suc(TARVBM *a, int elem){
  if(!a) return -1;
  if(max_suc(a, elem)) return __INT_MAX__;
...
TLSEINT *misc(TARVBM *a, int elem){
  ...
  int i = 0;
  while(!aux->folha){
    l = TLSEINT_insere(l, aux->chave[0], aux->chave[aux->nchaves-1]);
    while(aux->chave[i]<elem) i++;
    aux = aux->filho[i];
    i = 0;
  }
  while(aux->chave[i]!=elem) i++;
  l = TLSEINT_insere(l, aux->chave[0], aux->chave[i]);
  TLSEINT_imprime(l);
  return l;
}
