...
int suc(TARVB *a, int elem){
  ...
  int i = 0;
  while(i<a->nchaves) i++;
  if(a->folha && a->chave[i-1] != elem) return -1;
  if(a->folha && a->chave[i-1] == elem) return __INT_MAX__;
  return suc(a->filho[i], elem);
}
...
TARVB *maior(TARVB *a){
  ...
  int i = 0;
  while(i<a->nchaves) i++;
  if(a->folha) return TARVB_Busca(a, a->chave[i-1]);
  return maior(a->filho[i]);
}

TARVB *menor(TARVB *a){
  if(!a) return a;
  if(a->folha) return TARVB_Busca(a, a->chave[0]);
...
TARVB *retira_pares(TARVB *a){
  ...
  for(i = 0; i<a->nchaves; i++){
    if(!a->folha) a->filho[i] = retira_pares(a->filho[i]);
    if(a->chave[i]%2==0) a = TARVB_Retira(a, a->chave[i], sizeof(a->filho[i])/(sizeof(TARVB)*2));
  }
  if(!a->folha) a->filho[i] = retira_pares(a->filho[i]);
  return a;
}
...
int ni(TARVB *a){
  ...
  if(!a->folha){
    i++;
    int j;
    for (j = 0; j<=a->nchaves; j++) i+=ni(a->filho[i]);
  }
  return i;
}
...
int nf(TARVB *a){
  ...
  if(!a->folha){
    int j;
    for(j=0; j<=a->nchaves;j++) i += nf(a->filho[i]);
  } else i++;
  return i;
}
...
void quant_mult(TARVB *a, int n, int m, int *count){
  if(!a) return;
  TARVB *b = a;
  for(int i = 0; i<=b->nchaves; i++){
    quant_mult(b->filho[i],n,m,count);
    if(i<b->nchaves){
      if(b->chave[i]%n==0 && b->chave[i]%m==0) {
        (*count)++;
      }
    }
  }
...
void preenche_vet(TARVB *a, int *vet, int n, int m, int *pos){
  if(!a) return;
  int i;
  TARVB *b = a;
  for(i = 0; i<=b->nchaves; i++){
    preenche_vet(b->filho[i], vet, n, m, pos);
    if(i<b->nchaves){
      if(b->chave[i]%n==0 && b->chave[i]%m==0){
        vet[(*pos)] = b->chave[i];
        (*pos)++;
...
int *mult_MN(TARVB *a, int n, int m, int *tam){
  (*tam) = 0;
  if(!a) return tam;
  quant_mult(a,n,m,tam);
  int *vet = (int*) malloc(sizeof(int)*(*tam));
  int pos = 0;
  preenche_vet(a,vet,n,m,&pos);
  return vet;
}