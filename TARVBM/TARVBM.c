...
TARVBM *maior(TARVBM *a){
  if(!a) return a;
  TARVBM *b = a;
  while(!b->folha) b = b->filho[b->nchaves];
  return b;
}
...
TARVBM *retira_pares(TARVBM *a){
  ...
  while(!b->folha) b = b->filho[0];
  while(b){
    for(int i = 0; i<b->nchaves; i++) if(a->chave[i]%2==0) a = TARVBM_retira(a, a->chave[i], 2);
    TARVBM_imprime(a);
    b = b->prox;
  }
  return a;
...
int suc(TARVBM *a, int elem){
  if(!a) return 0;
  while(!a->folha) a = a->filho[a->nchaves];
  if(a->chave[a->nchaves-1] == elem) return __INT_MAX__;
  return 0;
}
...
int ni(TARVBM *a){
  ...
  if(!a->folha){
    c++;
    for(int j = 0; j<=a->nchaves; j++) c += ni(a->filho[j]);
  }
  return c;
}
...
int nf(TARVBM *a){
  if(!a) return 0;
  int c = 0;
  if(!a->folha) for(int i = 0; i<=a->nchaves; i++) c+= nf(a->filho[i]);
  else c++;
  return c;
}
